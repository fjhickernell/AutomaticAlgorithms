...
Let $b$ be prime number, and let $\F_{n}:=\{0, \ldots, n-1\}$ denote the set of the first $n$ non-negative integers for any $n \in \N$. The aim is to construct a sequence of embedded node sets with $b^m$ points:
\[
\{0\}=\cp_0\subset \cp_1 \dots\subset\cp_m=\{\bsz_i\}_{i=0}^{b^m-1} \subset\dots\subset\cp_\infty=\{\bsz_i\}_{i=0}^{\infty}.
\]
Moreover, the sequence $\bsz_1, \bsz_b, \bsz_{b^2},  \ldots \in \cube$ is chosen such that 
...
\begin{gather}
i \bsz_{b^{m-1}} \bmod {1} \ne \bszero \qquad \forall i\in \F_{b^m},\ m \in \N \\
...
&= \{\bsk \in \Z^d : \ip{\bsk}{\bsz_{b^{m-1}}} = 0 \}.\label{dualdef}
\end{align}
By this definition $\cp^{\perp}_{0}=\Z^d$ and the properties \eqref{cpinfvector}, \eqref{bilinear}, and \eqref{latpropd}, imply also that the $\cp^{\perp}_m$ are nested subgroups with
\begin{equation}\label{dualemb}
\Z^d=\cp^{\perp}_{0}\supseteq\dots\supseteq\cp^{\perp}_{m}\supseteq\dots\supseteq\cp^{\perp}_{\infty}=\{0\}.
\end{equation}

For finite node $m$ the dual lattice $\cp^{\perp}_{m}$ contains infinitely many points. Figure \ref{Latticefig} shows an example of a rank-1 lattice node set with $64$ points in dimension $3$ and its dual lattice. The parameters defining this node set are is $b=2$, $m=6$, and $\bsz_{32}=(1,3,7)/64$.
\begin{figure}[h!]
\centering
...
a) & b)
\end{tabular}
\caption{Plots of a) the node set $\cp_6$, and b) some of the dual lattice, $\cp_6^{perp} \cup [-10,10]^3$.}\label{Latticefig}
\end{figure}
...
\]
The complex expontential functions, $\E^{2 \pi \sqrt{-1} \ip{\bsk}{\cdot}}$,  $\bsk \in \Z^d$ form a  the complete orthonormal \emph{basis} for $\Lebesgue_2(\cube)$.  Any function in $\Lebesgue_2$ may be written in series form as
...
This property of the dual lattice is used below to describe the absolute error of a shifted rank-1 lattice cubature rule in terms of the Fourier coefficients for wavenumbers in the dual lattice. The  cubature rule is defined as
\begin{equation} \label{cubaturedef}
\hI_m(f) := \frac{1}{b^m} \sum_{i=0}^{b^m-1} f(\bsz_i \oplus\bsDelta),  \qquad \bsDelta \in \cube, \quad m \in \N_0.
\end{equation}
...
\nonumber
& = \Biggabs {\sum_{\bsk \in \cp_m^{\perp}\setminus \{\bszero\} } \hf(\bsk)  \E^{2 \pi \sqrt{-1} \ip{\bsk}{\bsDelta}} } \\
& \le \sum_{\bsk \in \cp_m^{\perp}\setminus \{\bszero\} } \abs{\hf(\bsk)}. \label{err1}
\end{align}

Adaptive Algorithm \ref{adapalgo} that we construct in section \ref{algorithmsection} is based on this expression.  However, since the true Fourier coefficients are unknown, they must be approximated by the discrete Fourier coefficients, which are defined as follows:
...
\section{Fast Fourier Transform Adapted to Rank-1 Lattices}\label{FFT}

The next task is to define the map $\tnu_m : \Z^d \to \F_{b^m}$ that facilitates the calculation of the discrete Fourier transform introduced above by a fast algorithm explained below.  This map depends on the choice of the embedded rank-1 lattice node sets defined in \eqref{zbmdef} and \eqref{zidef}.
\begin{definition} \label{numapdef} For every $\bsk \in \Z^d$, let
\begin{subequations} \label{numapdefeq}
\begin{gather}
\hnu_0(\bsk) = b\ip{\bsk}{\bsz_{1}}=\bsk^T\bsa_1 \bmod b, \\ 
\hnu_m(\bsk)=b\ip{\bsk}{\bsz_{b^m}}-\ip{\bsk}{\bsz_{b^{m-1}}}=\bsk^T\bsa_m \bmod b, \qquad m \in \N, \\
...
& = \sum_{\ell=0}^{m-1} i_\ell \sum_{j=0}^{\ell} \hnu_j(\bsk) b^{j-\ell-1} \pmod 1=\sum_{\ell=0}^{m-1} i_\ell \tnu_{\ell+1}(\bsk)  b^{-\ell-1} \pmod 1.
\end{align*}
...
\nonumber
\tf_m(\bsk)
&:= \hI_m\left( \E^{-2 \pi \sqrt{-1} \ip{\bsk}{\cdot}} f(\cdot) \right) = \frac{1}{b^m}\sum_{i=1}^{b^m-1} \E^{-2 \pi \sqrt{-1} \ip{\bsk}{\bsz_{i}\oplus \bsDelta}} y_i = \E^{-2 \pi \sqrt{-1} \ip{\bsk}{\bsDelta}} Y_{\tnu(\bsk)}
...
\item $\tnu_m(\bsk)=\tnu_m(\bsl)$ iff $\bsk-\bsl \in \cp_m^{\perp}$, 
\item 
\item $\bsk$
\end{enumerate}
\end{lemma}
