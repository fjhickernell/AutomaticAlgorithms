...
=======
\abstract{Quasi-Monte Carlo methods are useful for numerical integration of multivariate functions. However, the results obtained by these methods lack guarantees based on function values. In this article, we propose an error bound based on the decay rate of discrete Fourier coefficients of the integrand. If this decay is fast, the integrand is flatter and accuracy is higher. We focused on Rank-1 Lattices, because their algebraic structure facilitates an error analysis based on a Fourier decomposition of the integrand. Moreover, the error bound can be computed via a one-dimensional Fast Fourier transform . This leads to a guaranteed adaptive automatic cubature algorithm with cost $\Order(mb^m)$, where $b$ is the base under which our points are built and $b^m$ is the number of data points.}
>>>>>>> 02c03380ed927cdd39aa0be8b9718e512bb78f64
