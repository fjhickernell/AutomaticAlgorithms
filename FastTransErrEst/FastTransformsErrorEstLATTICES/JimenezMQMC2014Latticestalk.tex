...
\abstract{Quasi-Monte Carlo methods are useful for numerical integration of multivariate functions. However, the results obtained by these methods lack guaranties based on function values. In this article, we propose an error bound based on the decay of discrete Fourier coefficients of the integrand. If this decay is fast, the integrand is flatter and accuracy is higher. We focused on Rank-1 Lattices, because their algebraic structure facilitates an error analysis based on a Fourier decomposition of the integrand. Moreover, the error bound can be computed via a one-dimensional Fast Fourier transform . This leads to a guaranteed adaptive automatic cubature algorithm with cost $\Order(mb^m)$, where $b^m$ is the number of data points.}
