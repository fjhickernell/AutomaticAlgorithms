...
\begin{gather}
\{\vt_{b-1}, \vt_{b^2-1}, \vt_{b^3-1}, \ldots\} \text{ are linearly independent}, \\
\vt_i = \sum_{\ell=0}^{\infty} i_\ell \vt_{b^\ell-1}, \qquad \text{where }\ai=(i_0, i_1, i_2, \ldots ) \in \bbF_b^\infty, \\
...
&= \{\vk \in \bbK : \ip{\vk}{\vt_{b^{\ell}-1}} = 0, \ \ell=0, \ldots, m-1\}.
\end{align*}
...
b\vt_{b^m-1}\pmod 1=\vt_{b^{m-1}-1}
\end{equation}
 This gives us another condition on $j_m$, $\vz\frac{j_m-j_{m-1}}{b^{m-1}} \pmod 1 = 0$.
...
\hnu_0(\vk) = b\ip{\vk}{\vt_{1}}, \qquad \hnu_m(\vk)=b\ip{\vk}{\vt_{b^m-1}}-\ip{\vk}{\vt_{b^{m-1}-1}}, \quad m \in \naturals, \\
...
\item By construction, $\hnu_m(\vk)\in (-1,b)$. Using the assumption \eqref{assumgen}, $\hnu_m(\vk)\pmod 1=\vk^Tb\vt_{b^m-1}\pmod 1-\vk^T\vt_{b^{m-1}-1}\pmod 1$=0. Then, $\hnu_m(\vk)\in (-1,b)\cap\integers=\{0,\dots,b-1\}$.
\item Follows from Definition \ref{numapdef} and \eqref{bilinearlinkprop}:
\begin{multline*}
