...
\begin{equation}\label{origconeinteg}
...
\begin{equation} \label{newcone}
\hcc_{\htau}:= \{ f \in \cv^1 :  \Var(f'-A_n(f)') \le \htau(n) \Var(f') \ \forall n \in \ci \},
\end{equation}
Here $\htau : \ci \to [0,1]$ is some specified non-increasing function that defines the cone.  Integrands in this cone satisfy the following useful properties.
...
\item[Step 1.\ Bound {$\Var(f')$} and check for convergence.] Compute $F_{n_i}(f)$ in \eqref{Fnormalg}.  Check whether $n_i$ is large enough to satisfy the error tolerance, i.e.
    \begin{equation*}
      \htau(n) F_{n_i}(f) \le 8 (n-1)^2 \varepsilon.
    \end{equation*}
...
The cone defined in \eqref{newcone} makes Algorithm \ref{newconealgo} work.  In this section we show that it contains and is contained in other cones that might be more intuitive.  The family of cones of interest are defined as
