...
\hcc_{\htau}:= \{ f \in \cv^1 :  \Var(f'-A_n(f)') \le \htau(n) F_n(f) \ \forall n \in \ci \},
\end{equation}
Here $\htau : \ci \to [0,\infty]$ is some specified non-increasing function that defines the cone.  
...
\item[Step 2.\ Increase the number of trapezoids.]  If the above condition is false, choose
$$
n_{i+1}=\min \{ n \in \naturals : (n-1)/(n_i-1) \in \{2, 3, \ldots \}, \  \htau(n) F_{n_i}(f) \le 8 (n-1)^2 \}.
$$
Go to Step 1.
\end{description}
\end{algo}
...
\]
which is why $\ttau(n) \le 2$ for all $n$.

\begin{theorem} Given the function $\htau:\ci \to [0,\infty]$, suppose that $\ttau_j : \ci \to [0,2]$, $j=1,2$ satisfy the inequality
\[
\ttau_1(n) \le \frac{\htau(n)}{1+\htau(n)} \le \min(2,\htau(n)) \le \ttau_2(n) \qquad \forall  n \in \ci.
\]
...
Integrands in this cone satisfy the following useful properties.

\begin{lem}\label{newconeprop} Let $N_{\min} = \min \{n\in \ci : \htau(n) < 1\}$, and $\hci =\{N_{\min}, N_{\min}+1, \ldots\}$.  For $f \in \hcc_{\htau}$ it follows that
\[
\Var(f') \le \frac{F_n(f)}{1-\htau(n)}, \quad 
\Var(f'-A_n(f)') \le \frac{\htau(n) F_n(f)}{1-\htau(n)} ,\qquad
\forall n \ge \hci.
\]
\end{lem}
\begin{proof}  Because $f \mapsto \Var(f')$ is a semi-norm, it follows that for all $f \in \hcc_{\htau}$ and all $n \in \ci$,
\[
\Var(f') \le \Var(f'-A_n(f)') + \Var(A_n(f)') \le \htau(n) \Var(f') + F_n(f).
\]
Rearranging the terms in this inequality leads to the desired results provided that $\htau(n) < 1$.
\end{proof}


The cone defined in \eqref{Fcone} makes Algorithm \ref{newconealgo} work.  In this section we show that it contains and is contained in other cones that might be more intuitive.  The family of cones of interest are defined as
\begin{equation}\label{origconefamilyinteg}
\cc_{\ttau}:=\{f\in \cv^{1}:\Var(f'-A_n(f)')\leq\ttau(n)\norm[1]{f'-A_n(f)'}, \ n \in  \ci\},
\end{equation}
where $\ttau:\ci \to (1,\infty)$.
Under this definition $\cc_{\tau}$ corresponds to defining $\ttau(2)=\tau$, $\ttau(n)=\infty$ for $n>2$.
To facilitate the comparison of $\cc_{\ttau}$ and $\hcc_{\htau}$ we prove the following lemma.

\begin{lem} \label{FnFtnlem}  Let $n=p(m-1)+1$ for some positive integer $p$.  Then 
\[
F_n(f) \le 2????\norm[1]{f'-A_n(f)'}, \qquad   \forall f \in \cv^1
\]
\end{lem}
\begin{proof}
For all $f$
\begin{align} 
\nonumber
F_{n}(f) &= (n-1)\sum_{i=1}^{n-2} \biggabs{f\left(\frac{i-1}{n-1}\right) - 2 f\left(\frac{i}{n-1}\right)+f\left(\frac{i+1}{n-1}\right)}\\
\nonumber
&= (n-1)\sum_{k=1}^{m} \Bigl [\sum_{j=1}^{p-1} \biggl \lvert f\left(\frac{k-1}{m-1}+\frac{j-1}{p(m-1)}\right) - 2 f\left(\frac{k-1}{m-1}+\frac{j}{p(m-1)}\right)\\
\nonumber
& \qquad \qquad +f\left(\frac{k-1}{m-1}+\frac{j+1}{p(m-1)}\right) \biggr \rvert \\
\nonumber
&= (n-1)\sum_{k=1}^{m} \sum_{j=1}^{p-2} \bigabs{f(x_{p(k-1)+j}) - 2 f(x_{p(k-1)+j+1})+f(x_{p(k-1)+j+2})}\\
\nonumber
...
\nonumber
&\le 2 \sum_{i=1}^{n-1}\biggabs{f(x_{i+1})-f(x_{i}) - \frac{f(1)-f(0)}{n-1}}\\
\nonumber
&=2(n-1) \bignorm[1]{A_n(f)'-f(1)+f(0)} \\
&\le 2(n-1) \bignorm[1]{f'-f(1)+f(0)} . \label{Fnupbd}
\end{align}
\end{proof}


Recall from \cite{HicEtal14b} that 
\begin{align*}
\norm[1]{f'-f(1)+f(0)} &\ge \tF_n(f):= \bignorm[1]{A_n(f)'-A_2(f)'} \\
&=\sum_{i=1}^{n-1}\left|f(x_{i+1})-f(x_{i}) - \frac{f(1)-f(0)}{n-1}\right|.
\end{align*}
Morever,  for all $f \in \cv^1$
\begin{align} 
\nonumber
...
\input{univariate_integration_i.tex}
