...
\end{equation}
for $h\le h_0=c_0/\ell_0$, where the fill distance
\begin{equation*}
...
In fact we can obtain the same upper bound for $\cl_p$-norm of the error for any $p\ge1$ by
\begin{equation*}
\norm[\cl_p(\cx)]{f-A_nf}:=\left(\int_\cx|f(\vx)-(A_nf)(\vx)|^p\rho(\vx)\dif\vx\right)^{1/p}\le e^{c\ln h/h}\norm[\ch]f.
\end{equation*}

Next we assume that $f\in\cc_\tau:=\left\{g:g\in\ch,D^\alpha g\in\cl_\infty(\cx),\norm[\ch]g\le\tau\norm[\cl_\infty(\cx)]{D^\alpha g}\right\}$ and use this cone condition obtain an upper bound for $\norm[\ch]f$.
...
It follows that
\begin{equation*}
\left|\norm[\cl_\infty(\cx)]{Df}-\norm[\cl_\infty(\cx)]{D(A_nf)}\right|\le\norm[\cl_\infty(\cx)]{Df-D(A_nf)}\le c^{(1)}h\norm[\ch]f\le c^{(1)}h\tau\norm[\cl_\infty(\cx)]{Df},
\end{equation*}
which implies
\begin{equation*}
\norm[\ch]f\le\tau\norm[\cl_\infty(\cx)]{Df}\le\frac\tau{1-\tau c^{(1)}h}\norm[\cl_\infty(\cx)]{D(A_nf)}.
\end{equation*}
