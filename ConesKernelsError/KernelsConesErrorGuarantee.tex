...
\end{equation*}
and the constant $c_0:=R/(3\alpha_d)$, where $\alpha_d$ is defined by
\begin{equation*}
\alpha_1=2,\ \ \ \mbox{and}\ \ \ \alpha_j=2j(1+\alpha_{j-1}),\ \ \ \mbox{for}\ \ \ j=2,3,\ldots.
\end{equation*}
The constant $c$ in \eqref{spectral} is given by $c:=\min\{c_0/2,1/(eC_3)\}/2$, where $C_3:=C_1e^{C_2}$, $C_1:=576\g d\alpha_d^2$, and $C_2:=\ln4+8d\alpha_d$.
