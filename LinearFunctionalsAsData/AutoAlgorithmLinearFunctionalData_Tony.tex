...
\begin{equation}
         \sigma_{k^*}(f) \le \frac{\varepsilon}{\norm[\pout]{\underline{\boldsymbol{\gamma}}}} < \sigma_{k^*-1}(f)
\end{equation}
...
\begin{prop}
The algorithm proposed is optimal.
\end{prop}
\begin{proof}
Easy to prove taking the function in the boundary of $\cc$. This holds for the equality so:
\begin{align*}
\sigma_{0}(f) \norm[\pout]{\underline{\gamma_k}} \le \varepsilon &< \sigma_{0}(f) \norm[\pout]{\underline{\gamma_{k-1}}} \\
\varepsilon & < \norm[\pout]{S-A_{n_{k^*-1}}}=\sigma_{0}(f) \norm[\pout]{\underline{\gamma_{k-1}}} \\
\norm[\pout]{S-A_{n^*}} \le \varepsilon &
\end{align*}

This means that $\frac{n_{k^*-1}}{n^*}<1$.
\end{proof}

We can extend that to $\frac{n_{k^*-i}}{n^*}<C^i$ where $C=\sup_i\frac{n_{i+1}}{n_i}$.
