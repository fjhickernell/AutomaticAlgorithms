...
\begin{algo}[Univariate Function Recovery] \label{multistageapproalgo}
...
\begin{theorem} \label{multistageappxthm} Let $A \in \ca(\cc_{\tau}, \reals, \APP, \Lambda^{\std})$ be the automatic linear spline defined by Algorithm \ref{multistageapproalgo}, and let $\tau$, $n_1$, and $\varepsilon$ be the inputs and parameters described there. Let $\cc_\tau$ be the cone of functions defined in \eqref{coneappxdef}.  Then it follows that Algorithm \ref{multistageintegalgo} is successful for all functions in $\cc_{\tau}$,  i.e.,  $\abs{f - A(f,\varepsilon)} \le \varepsilon$.  Moreover, the cost of this algorithm is bounded above as follows:
\begin{multline}
...
\begin{theorem} \label{complowbdappr} For $\sigma>0$ let $\cb_{\sigma}=\{f \in \cw^{2,\infty} : \norm[\infty]{f'} \le \sigma\}$.  The complexity of function recovery on this ball is
\begin{equation*}
\comp(\varepsilon,\ca(\cb_{\sigma},\cl_\infty,\APP,\Lambda^{\std}),\cb_{s}) \ge \sqrt{\frac{\min(s,\sigma)}{16 \varepsilon}} -1 .
\end{equation*}
Furthermore, Algorithm \ref{nonadaptalgo} using linear splines is optimal in the sense of Theorem \ref{optimalprop}.
...
\begin{equation*}
\comp(\varepsilon,\ca(\cc_{\tau},\cl_\infty,\APP,\Lambda^{\std}),\cb_{s}) \ge \sqrt{\frac{(\tau-2)s}{32 \tau \varepsilon}} -1 .
\end{equation*}
