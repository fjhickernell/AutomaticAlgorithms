The algorithms used in this section and the next are all based on linear splines on $[0,1]$.  The node set and the linear spline algorithm using $n$ function values are defined for $n \in \mathcal{I}=\{2,3,\ldots\}$ as follows:
...
The space of input functions is $\cf=\mathcal{V}^{1}$, the space of functions whose first derivatives have finite variation.  The general definitions of these relevant norms and spaces are as follows:
...
f(x)=
\begin{cases}
\displaystyle  \frac{b}{2}[(x-z)^2-(x-z-a)^2-(x-z+a)^2+4a^2], & z-a\leq x\leq z+a,\\[2ex]
\displaystyle  0, & \text{otherwise}.
\end{cases}.
\end{equation}
with  $\log_{10}(a) \sim \cu[-4,-1]$, $z \sim \cu[2a,1-2a]$, and $b=3/(5a^2)$ is chosen to make $\int_0^1 f(x) \, \dif x = 1$.
...
\toprule
&$10$ & $0\% \rightarrow  29\% $ & $29\%$ & $0\%$ & $71\%$  \\
Algorithm \ref{multistageintegalgo}
 &$100$ & $33 \% \rightarrow 56\% $ & $54\%$ & $2\%$ & $46\%$ \\
&$1000$ & $60\% \rightarrow 88\% $& $63\%$ & $4\%$ &$37\%$ \\
\midrule
{\tt quad} & 3\%\\
{\tt quadgk} & 60\%\\
{\tt chebfun} & 2\%\\
