...
One would prefer an algorithm where the costs for input functions with the same $\cg$-semi-norm do not vary widely.  To understand this one defines the  \emph{minimum} cost:
\begin{equation*}
...
\le n_G+ \max\left(\tN_{\max}, N_{A}\left(\frac{\varepsilon}{\fC \fc \Gnorm{f}} \right)\right), \\
\mincost(A,\cn,\varepsilon,N_{\max},\Gnorm{f})
\ge n_G+ \min\left(\tN_{\max}, N_{A}\left(\frac{\varepsilon}{\Gnorm{f}} \right) \right).
\end{gather}
\end{subequations}
