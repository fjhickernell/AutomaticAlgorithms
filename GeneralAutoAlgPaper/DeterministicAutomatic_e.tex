...
The automatic, adaptive algorithms developed here are valid for subsets of $\cf$ that may be unbounded, e.g., cones of functions, $\cc_{\tau}$, as described in Section \ref{conesubsec}, or their subsets, $\cn$.  The design of the algorithm depends on the definition of $\cn$.  The computational cost of the adaptive algorithm for $f$ in $\cn$ may be quite large or even arbitrarily large, depending on the $\cf$-semi-norm of $f$.  We want to find upper and lower bounds on the cost of the algorithm that take into account more information about $f$, say that it lies in some ball. However, the definition of the ball is not used by the algorithm, but only as a device to measure computational cost. Let $\{\cb_{\sigma}\}_{\sigma \ge 0}$ be some collection of nested subsets of $\cf$, namely, $\cb_{\sigma_1} \subseteq \cb_{\sigma_2}$ for $\sigma_1 \le \sigma_2$, and $\cup_{\sigma \ge 0} \cb_{\sigma} = \cf$.  For automatic algorithms defined with respect to some set of functions $\cn \subset \cf$, we define the \emph{maximum} cost of the algorithm relative to $\cb_{\sigma} \subset \cf$ as follows:
\begin{equation*}
\maxcost(A,\cn,\varepsilon,N_{\max},\cb_{\sigma})
= \sup \{ \cost(A,f;\varepsilon,N_{\max}) : f \in \cn \cap \cb_{\sigma} \} .
\end{equation*}
Here the set $\cn$ is allowed to depend on the algorithm inputs, $\varepsilon$ and $N_{\max}$, but not on $\sigma$.  Again we stress that $A$ knows $\cn$ but not $\cb$.
...
= \inf \{ \cost(A,f;\varepsilon,N_{\max}) : f \in \cn\setminus \cb_{\sigma} \} .
\end{equation*}
An algorithm is said to have \emph{stable computational cost} if 
\begin{equation*}
\sup_{\varepsilon, \sigma > 0} \frac{\maxcost(A,\cn,\varepsilon,N_{\max},\cb_\sigma)}{\mincost(A,\cn,\varepsilon,N_{\max},\cb_\sigma)} < \infty
\end{equation*}
...
\end{equation}
where $\tau_{\min}$ is some positive constant.  Here, the subspace of functions in $\cg$ with vanishing $\cg$-semi-norm, $\cg_0=\{f \in \cg : \Gnorm{f}=0\}$, is assumed to be a finite dimensional subspace of $\cf$ as well, and by the above condition must be identical to the subspace of of functions with vanishing $\cf$-semi-norm.

Given $\tau>0$, let $\cc_{\tau} \subset \cf$ denote a \emph{cone} of functions whose $\cf$-semi-norms are no greater than $\tau$ times their $\cg$-semi-norms, as defined in \eqref{conedef}.  Note that one must choose $\tau \ge \tau_{\min}$ for $\cc_{\tau}$ to contain functions outside $\cg_0$.
