...
\begin{gather*}
\maxcost(A,\cn,\varepsilon,\cb_\sigma)
= \sup \{ \cost(A,f,\varepsilon) : f \in \cn \cap \cb_{\sigma} \}, \\ \mincost(A,\cn,\varepsilon,\cb_\sigma)
= \inf \left \{ \cost(A,f,\varepsilon) : f \in \cn\setminus \bigcup_{\sigma'<\sigma}\cb_{\sigma'} \right\} .
\end{gather*}
...
\sup_{\varepsilon, \sigma > 0} \frac{\maxcost(A,\cn,\varepsilon,\cb_\sigma)}{\max(1,\mincost(A,\cn,\varepsilon, \cb_\sigma))} < \infty.
\end{equation} 
...
\comp(\varepsilon,\ca(\cn,\cg,S,\Lambda),\cb_\sigma) \\
 = \inf\left\{\maxcost(A,\cn,\varepsilon,\cb_\sigma) : A \in \ca(\cn,\cg,S,\Lambda), \right. \\
 \left . \norm[\cg]{S(f)-A(f,\varepsilon)} \le \varepsilon \ \ \forall \varepsilon \ge 0 \right \} \in \natzero.
\end{multline}
Here the infimum of an empty set is defined to be $\infty$.  The an adaptive algorithm is \emph{optimal} if its cost tracks that of the best possible possible algorithm, i.e.,
\begin{equation} \label{nearoptdef}
\sup_{\varepsilon,\sigma > 0} \frac{\maxcost(A,\cn,\varepsilon,\cb_\sigma)} {\max(1,\comp(\varepsilon,\ca(\cn,\cg,S,\Lambda),\cb_\sigma))} <\infty.
\end{equation}
...
\[
\maxcost(A,\cb_{\sigma},\varepsilon,\cb_{\sigma}) = \mincost(A,\cb_{\sigma},\varepsilon,\cb_{\sigma}) = \cost(A,f,\varepsilon) = h^{-1}(\varepsilon/\sigma).
\]
The optimality of Algorithm \ref{nonadaptalgo} depends on whether $h$ satisfies condition ??? below.
...
\begin{algo} \label{twostagedetalgo} {\bf (Adaptive, Automatic, Two-Stage).} Let $\cf$, $\Fnorm{\cdot}$, $\Ftnorm{\cdot}$, $\cg$, $\Gnorm{\cdot}$, and $S$ be as described above.  Let $\tau$ be a fixed positive number, and let $\tF_{n_{\tF}}$ be an algorithm as described in Lemma \ref{Gnormlem} with cost $n_{\tF}$ satisfying $h_{+}(n_{\tF}) < 1/\tau$.
...
\begin{theorem}  \label{TwoStageDetermThm}  Under the assumptions of Algorithm  \ref{twostagedetalgo}, let $\fc=\fc_{n_{\tF}}$ be defined as in \eqref{normdeflate}.
...
\begin{rem} \label{htilderem} It may also be possible to obtain an error upper bound in terms of weaker semi-norm, i.e., $\tildeh$ such that $\norm[\cg]{S(f)-A_n(f)} \le \tildeh(n) \Ftnorm{f}$. By following the argument leading to \eqref{Anerrbound}, it is clear that such an error bound is no use if
...
sufficient condition for \eqref{hbest} is finding some sequence of functions $\{f^*_n \in \cf\}_{n \in \ci}$ with non-zero $\cf$- and $\tcf$-semi-norms such that 
\begin{equation} \label{hbestverc}
h(n) \le \frac{\norm[\cg]{S(f^*_n)-A_n(f_n^*)}}{\Ftnorm{f_n^*}} \frac{\Ftnorm{f^*_n}-\tF_n(f^*_n)}{\Fnorm{f^*_n}}   \quad \forall n \in \ci,
\end{equation}
since the right hand side above is no greater than the right hand side in \eqref{hbestverb}. An advantage of \eqref{hbestverc} is that it does not require $\tildeh$ to be known explicitly. Finally, suppose that one can construct a sequence of functions $\{f^*_n \in \cf\}_{n \in \ci}$ \emph{with all vanishing data} for both the algorithms $A_n$ and $\tF_n$ that attains the upper bound on the error of $A_n$ with respect to the $\cf$-semi-norm, i.e.,
\begin{equation} \label{hbestverd}
h(n) = \frac{\norm[\cg]{S(f^*_n)}}{\Fnorm{f^*_n}}   \quad \forall n \in \ci.
\end{equation}
...
\begin{algo} \label{multistagealgo}  Let $\cf$, $\Fnorm{\cdot}$, $\Ftnorm{\cdot}$, $\cg$, $\Gnorm{\cdot}$, $S$, and $\tau$ be as described above.  Let the sequences of algorithms, $\{A_n\}_{n \in \ci}$ and  $\{\tF_n\}_{n \in \ci}$ be as described above.  Set $i=1$, and $n_1 = \min\{ n \in \ci : h_+(n) < 1/\tau\}$. For any positive error tolerance $\varepsilon$ and any input function $f$, do the following:
\begin{description}
...
\begin{theorem}  \label{MultiStageThm}  Under the assumptions of Algorithm \ref{multistagealgo}, let $r$ be the minimum cost multiple described in the paragraphs preceding that Algorithm \ref{MultiStageThm}.  
...
Consider fixed (semi-)normed linear spaces $\cf$ and $\cg$, as described above, and now assume that the solution operator $S: \cf \to \cg$ is \emph{linear}.  Let $\Lambda$ be the set of bounded linear functionals that can be used as data. Let $\ci$ be a subset of $\natzero$ satisfying \eqref{Inobiggap}.  Suppose that for any $n \in \ci$, and for all $\vL \in \Lambda^m$, satisfying $\$(\vL)\le n$, there exists an $f_1 \in \cf$, depending on $n$ and the $L_i$, with zero data, solution norm one, and $\cf$ semi-norms with known upper bounds, namely,
\begin{equation} \label{assumpfone}
\norm[\cg]{S(f_1)} = 1, \qquad \vL(f_1)= \vzero, \qquad
\Fnorm{f_1} \le \frac{1}{g(n)}, 
\end{equation}
for some positive, non-increasing function $g$ defined on $\ci$ with $\inf_{n \in \ci} g(n)=0$.  Let $g^{-1}$ be defined analogously to $h^{-1}$ in \eqref{hinvdef}. For example, one might have $g(n)=a n^{-p}$ with positive $a$ and $p$.  


{\bf Fix this here and below.}

Suppose $A$ is a successful automatic algorithm for the ball $\cb_{\sigma}$, i.e., $A \in \ca(\cb_{\sigma},\cg,S,\Lambda)$, and $\Gnorm{S(f)-A(f)}\le \varepsilon$ for all $f \in \cb_{\sigma}$, and suppose that for fixed $\varepsilon$, it is known that $n=\maxcost(A,\cb_{\sigma},\varepsilon,\cb_{\sigma})$.  Note that Since the data for the function $f_1$ are all zero, it follows that $A(f_1)=A(-f_1)$ for any algorithm, $A$, adaptive or not, that is based on the information $\vL(f_1)$.  Then, by the triangle inequality and \eqref{assumpfone} the error for one of the fooling functions $\pm f_1$ must be at least one:
\begin{align*}
\MoveEqLeft{\max(\norm[\cg]{S(f_1)-A(f_1)},\norm[\cg]{S(-f_1)-A(-f_1)})} \\
& = \max(\norm[\cg]{S(f_1)-A(f_1)},\norm[\cg]{-S(f_1)-A(f_1)})\\
& \ge \frac{1}{2} \left[ \norm[\cg]{S(f_1)-A(f_1)}+ \norm[\cg]{S(f_1)+A(f_1)} \right] \\
& \ge \frac{1}{2} \norm[\cg]{[S(f_1)-A(f_1)]+[S(f_1)+A(f_1)]} = \norm[\cg]{S(f_1)}=1.
\end{align*}
...
\begin{equation*}
\comp(\varepsilon,\ca(\cb_{\sigma},\cg,S,\Lambda)) \ge
g ^{-1}(\varepsilon).
\end{equation*}
...
\begin{prop} \label{optimalprop} Suppose that there exist non-adaptive algorithms $\{A_n\}_{n \in \ci}$, with $A_n  \in \ca_{\fix}(\cf,\cg,S,\Lambda)$, for which the upper error bounds satisfy \eqref{algseqerrbd} for known $h$.  If 
...
\begin{align*}
\sup_{0 < \varepsilon \le 1} \frac{\min\{n \in \ci : \err(A_n,\cf,\cg,S) \le \varepsilon\}}{\comp(\varepsilon,\ca_{\fix}(\cf,\cg,S,\Lambda))}
...
\begin{equation*}
\comp(\varepsilon,\ca(\cc_{\tau}\cap \cb_{\sigma},\cg,S,\Lambda),\infty,\cb_{\sigma}) 
...
\begin{align*}
\Fnorm{f_{\pm}} - \tau  \Ftnorm{f_{\pm}} & \le  c_{0}\Fnorm{f_0} + c_1 \Fnorm{f_1} - \tau (c_{0}\Ftnorm{f_0} - c_1 \Ftnorm{f_1}) \\
...
\begin{equation*}
\comp(\varepsilon,\ca(\cc_{\tau},\cg,S,\Lambda),\infty,\cb_{\sigma}) 
\ge g^{-1}\left(\frac{2 \tau \varepsilon}{\sigma(\tau-\tau_{\min})}\right).
\end{equation*} 
\end{cor}
...
\item the Banach spaces of input functions, $\cf$ and $\tcf$, and their semi-norms, 

\item the Banach space of outputs, $\cg$, and its norm,
...
\item a set of non-adaptive algorithms, $\{\tF_n\}_{n \in \ci}$, which approximate $\Ftnorm{\cdot}$, such that $\tF_n$ uses the same function data as $A_n$,
