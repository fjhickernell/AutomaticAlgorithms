function [Q,param]=adapttrapbetternorm(f,param)
...
bigdif=fpts(param.nmin+1)-fpts(1);
...
while ntrap<param.nmax
    absfirstdiff=sum(abs(diff(fpts)-bigdif/ntrap)); %estimate 1-norm of first derivative
    ...
    elseif ntrap > param.nmax/2 %cannot increase ntrap further
        ...
        mmult2=floor(param.nmax/ntrapold);
        mmult=max(2,min(mmult1,mmult2));
