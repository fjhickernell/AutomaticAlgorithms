...
One would like to have automatic, adaptive algorithms for numerical problems that are \emph{guaranteed} to provide answers whose errors do not exceed the user-specified error tolerance.  Such algorithms are generally lacking, even for fundamental problems such as integration and function recovery.  This article shows how to construct such algorithms. 

Specifically, we want algorithms with the following properties:
\begin{description}
\item[Guaranteed]--- the errors in the answers must be no greater than the user-specified \emph{error tolerance}, $\varepsilon$.
\item[Automatic]--- the computational effort depends on $\varepsilon$ and the function, but \emph{no additional user input is required}.
\item[Adaptive]--- the computational cost depends on some semi-norm of the input function, \emph{not input by the user, but reliably inferred from function data}.
\item[Optimal]--- the computational cost of the algorithm, as $\varepsilon \to 0$, does not exceed a constant multiple of the computational complexity of the problem (the computational cost of the best possible algorithm).
...
Most existing theory for numerical problems of interest starts with a Banach space, $\cf$, of input functions defined on some set $\cx$, and having a semi-norm, $\Fnorm{\cdot}$.  The definition of $(\cf,\Fnorm{\cdot})$ contains assumptions about smoothness, periodicity or other qualities of the input functions.  The mathematical problem of interest is defined by a \emph{solution operator} $S:\cf \to \ch$, where $\ch$ is some other Banach space with norm $\norm[\ch]{\cdot}$.  For integration, $\ch=\reals$, and for function approximation, $\ch$ is some superset of $\cf$, for example, $\cl_{\infty}(\cx)$. One often finds a sequence of algorithms, $\{A_n\}_{n \in \naturals}$, with computational cost $n$, which provides an approximate solution whose error can be bounded as
\begin{equation} \label{traditionerra}
\norm[\ch]{S(f)-A_n(f)} \le \frac{C_{\up} \Fnorm{f}}{n^p},
\end{equation}
where $C_{\up}$ and $p$ are known positive constants independent of $n$.  It is necessary for $A_n$ to be exact if the semi-norm of the input function vanishes, i.e., $S(f)=A_n(f)$ if $\Fnorm{f}=0$.  Error bound \eqref{traditionerra} allows one to compute an approximation that differs from the true solution by no  more than $\varepsilon$, as shown in the following algorithm and theorem. 

\begin{algo} \label{nonadaptalgo} {\bf (Automatic, Non-Adaptive).} Let $\cf$ and $\ch$ be Banach spaces as described above, let $S$ be the solution operator, and let $\sigma$ be a fixed positive number.  Let $\{A_n\}_{n \in \naturals}$ be a sequence of algorithms as described above such that error bound \eqref{traditionerra} holds.  Let  $\varepsilon$ be a positive error tolerance and $f \in \cf$ be an input function.  Compute the computational cost needed to satisfy the error tolerance:
\begin{equation}
\label{costupbd}
...
\end{tikzpicture}
\caption{Diagram of the guaranteed, non-adaptive Algorithm \ref{nonadaptalgo}. \label{fig:NonadaptAlgo}}
\end{figure}

The semi-norm $\Fnorm{f}$ may be thought of as the \emph{difficulty of the problem}.  In practice, it is difficult to know a priori the maximum possible value of $\Fnorm{f}$, so having to determine $\sigma$ in advance is a practical drawback of the Algorithm \ref{nonadaptalgo}.  Since the computational cost of this algorithm depends on $\sigma$, not $\Fnorm{f}$, the cost does not decrease if $\Fnorm{f}$ is drastically smaller than $\sigma$.  Moreover, if Algorithm \ref{nonadaptalgo} works for $f$, it may not work for $af$, where $a$ is some constant greater than one.

Adaptive algorithms use function data to try to bound $\Fnorm{f}$ or estimate $\norm[\ch]{S(f)-A_n(f)}$, so that their computational cost can be adjusted accordingly. Unfortunately, the bounds on $\Fnorm{f}$ and the estimates of $\norm[\ch]{S(f)-A_n(f)}$ used by adaptive algorithms are typically heuristic or valid only in the limit of $n \to \infty$.  These approaches are not guaranteed to provide the answer to within the specified error tolerance for finite $n$.  

Here we show how to construct a \emph{rigorous} upper bound on $\Fnorm{f}$ from data, which then leads to a guaranteed automatic and adaptive algorithm.  The key idea is to consider functions lying in a \emph{cone}, not a ball.  A cone is a set with the property that any positive multiple of an element in the set is also in the set.  Let $\cg$ be some superspace of $\cf$ with a weaker semi-norm, and define the cone
...
\begin{itemize}
\item Since the $\cg$-semi-norm is weaker, it can be approximated by some algorithm $G_n(f)$, with $\Gnorm{f}-G(f) \le h_+(n)\Fnorm{f}$ for some known, non-increasing $h_+(n)$ that vanishes as $n \to \infty$.  One chooses a sample size $n_G$ such that $h_+(n_G) \le 1/\tau$.
...
\[
n=\cost(A_n)= \left \lceil\left(\frac{ C_{\up}\tau G_{n_G}(f)}{\varepsilon(1 - \tau h_+(n_G))}\right)^{1/p} \right \rceil.
\]

\item Often it happens that $G_{n_G}(f)$ does not overestimate the $\cg$-semi-norm of $f$.  Moreover, since $\Gnorm{\cdot}$ is a weaker semi-norm, it follows that $\tau_{\min} \Gnorm{f} \le \Fnorm{f}$.  Thus, one can be assured that the above criterion will be met for 
\[
n=\cost(A_n)\le \left \lceil\left(\frac{ C_{\up}\tau \Fnorm{f})}{\varepsilon \tau_{\min} (1 - \tau h_+(n_G))}\right)^{1/p} \right \rceil.
\]
Thus, the cost of this adaptive algorithm is at worst a constant multiple times the cost of the non-adaptive Algorithm \ref{nonadaptalgo} that requires an a priori upper bound on $\Fnorm{f}$.

\item On top of the above analysis, in this article we allow for a maximum computational cost budget, $N_{\max}$, that allows the user to specify how long he or she is willing to wait for an answer.

\end{itemize} 

Figure \ref{fig:AdaptAlgo} is a diagram of the automatic algorithm just outlined.  Examples of this approach are given in Algorithms \ref{twostagedetalgo}, \ref{multistagealgo}, \ref{multistageintegalgo}, and \ref{multistageapproalgo}.
...
cost $\displaystyle \asymp \left(\frac{\tau\Fnorm{f}}{\tau_{\min}\varepsilon}\right)^{1/p}$ }};
...
Section \ref{integsec} presents a concrete example of this approach for the problem of computing the integral $\int_0^1 f(x) \, \dif x$ where the derivatives of the integrands have finite total variation, i.e., $\Fnorm{f}=\Var(f')$. The trapezoidal rule based on $n$ function values ($n-1$ trapezoids) has an error no greater than $\varepsilon$ provided $n \ge 1 + \sqrt{\Var(f')/(8\varepsilon)}$ function values.  However, except for relatively simple integrands, an upper bound on $\Var(f')$ is unknown in practice.  

Following the procedure outlined above we choose a weaker semi-norm based on the $1$-norm of the first derivative, $\Gnorm{f} = \norm[1]{f'-f(1)+f(0)}$.  It can be verified that $2\norm[1]{f'-f(1)+f(0)} \le \Var(f')$ for all $f$ for which $\Var(f')$ is finite.  This weaker semi-norm may be estimated by an algorithm $G_n$ that takes the $\cg$-semi-norm of the piecewise linear spline for $f$.  The error for $G_n(f)$ is bounded by $\Var(f')/(2n-2)$.  Fixing the parameter $\tau > 2$, and assuming that $\norm[1]{f''} \le \tau \norm[1]{f'-f(1)+f(0)}$  (the cone condition), then leads to a guaranteed, automatic (adaptive) Algorithm \ref{multistageintegalgo} for approximating the integral to the desired accuracy with a computational cost that is no greater than $4+ \tau + \sqrt{\tau \Var(f')/(4\varepsilon)}$ (Theorem \ref{multistageintegthm}), where $\Var(f')$ is unknown a priori. Here $\tau$ represents is related to the minimum sample size, and $1/\tau$ represents a length scale of for possible spikes that one wishes to integrate accurately.
...
Section \ref{integsec} illustrates the general results in Sections \ref{genthmsec} and \ref{LowBoundSec} for the univariate integration problem.  An explicit multistage automatic algorithm based on the trapezoidal rule is presented, and its success and optimality are proved in Theorem \ref{multistageintegthm}.  Section \ref{approxsec}  presents analogous results for function approximation.  Common concerns about automatic and adaptive algorithms are answered in Section \ref{overcomesec}. The article ends with several suggestions for future work.
...
The set $\ca_{\fix}(\cf,\ch,S,\Lambda)$ contains non-adaptive algorithms, i.e., those algorithms for which neither the choice of the $L_i$ and the number of function data depend on the input function.  Furthermore, any $A \in \ca_{\fix}(\cf,\ch,S,\Lambda)$ is assumed to satisfy the following scale properties:
\begin{equation}
\label{algoscale}
\vL(cf) = c \vL(f), \quad 
...
\end{equation}
The above error criteria are normalized in the sense that the absolute error, $\norm[\ch]{S(f) -  A(f)}$, is measured with respect to the $\cf$-semi-norm of the input function. 

Our focus is on algorithms that use at least one function datum.  One may define an algorithm with zero cost $A_0: f \mapsto 0$, and this algorithm satisfies the error tolerance if the error tolerance exceeds the magnitude of the solution operator.  In other words,
\[
\norm[\ch]{S} := \sup_{\substack{f \in \cf\\ \Fnorm{f} \ne 0}}  \frac{\norm[\ch]{S(f)}} {\Fnorm{f}}, \qquad \norm[\ch]{S(f)-A_0(f)} \le \varepsilon \Fnorm{f} \text{ for } \varepsilon \ge \norm[\ch]{S}.
\]
For the sake of optimality of algorithms, we assume that $\varepsilon < \norm[\ch]{S}$ so that a nonzero cost is required to solve the problem. 


The complexity of a problem for this set of algorithms, $\ca_{\fix}(\cf,\ch,S,\Lambda)$, is defined as the cost of the cheapest algorithm that satisfies the specified error tolerance, $\varepsilon$:
\begin{multline} \label{fixcostcomplex}
\comp(\varepsilon,\ca_{\fix}(\cf,\ch,S,\Lambda)) \\
= \min\left\{\cost(A) : \err(A,\cf,\ch,S) \le \varepsilon, \ A \in \ca_{\fix}(\cf,\ch,S,\Lambda) \right \} \in \natzero.
\end{multline}
Here the infimum of an empty set is defined to be $\infty$.  So, to guarantee that $\norm[\ch]{S(f) -  A(f)} \le \varepsilon$ for all $f \in \cb_{\sigma}$, one needs an algorithm with a cost of at least 
\[
\comp(\varepsilon/\sigma,\ca_{\fix}(\cf,\ch,S,\Lambda)).
\]
This cost is non-decreasing as either $\varepsilon$ decreases or $\sigma$ increases.
...
\end{gather}
\end{subequations}
where the positive integer-valued index set, 
\begin{equation} \label{indexdef}
\ci=\{n_1, n_2, \ldots\} \subseteq \naturals \quad \text{with } n_i < n_{i+1}, \quad \text{satisfies } \sup_i \frac{n_{i+1}}{n_i} <\infty. 
\end{equation} 
...
\sup_{0 < \varepsilon < \norm[\ch]{S}} \frac{\min\{n \in \ci : \err(A_n,\cf,\ch,S) \le \varepsilon\}} {\comp(\varepsilon,\ca_{\fix}(\cf,\ch,S,\Lambda))} <\infty.
\end{equation}
...
\[
\cost(A,f;\varepsilon,N_{\max}) = \$(\vL) = \$(L_1) + \cdots +\$(L_m) \in \naturals.
\]
Here the cost must be positive, because the zero algorithm is excluded from consideration.  Because of the potentially adaptive nature of the algorithm, namely that $m$ may depend on $f$, it follows that the cost may depend on $f$ as well as $A$. The input parameter $N_{\max}$ tells the algorithm to ensure that $\cost(A,f;\varepsilon,N_{\max}) \le N_{\max}$ for all $f$ and $\varepsilon$.  This is a practical consideration since the user does not want to wait indefinitely for an answer. 

The automatic, adaptive algorithms developed here are designed to be valid for some $\cn \subset \cf$ that may be unbounded, e.g., a cone or a subset of a cone.  Since $\cn$ is unbounded, the computational cost of the adaptive algorithm $\cn$ may vary over a large range, depending on the unknown, but estimated $\Fnorm{f}$.  To understand this dependence on $\Fnorm{f}$, we define the \emph{maximum} and \emph{minimum} costs of the algorithm relative to $\cb_{\sigma}$, the $\cf$-semi-norm ball defined in \eqref{balldef}, as follows:
\begin{gather*}
\maxcost(A,\cn,\varepsilon,N_{\max},\sigma)
= \max \{ \cost(A,f;\varepsilon,N_{\max}) : f \in \cn \cap \cb_{\sigma} \}, \\ \mincost(A,\cn,\varepsilon,N_{\max},\sigma)
= \min \left \{ \cost(A,f;\varepsilon,N_{\max}) : f \in \cn\setminus \bigcup_{\sigma'<\sigma}\cb_{\sigma'} \right\} .
\end{gather*}
Again we stress that $A$ knows that $f \in \cn$ but not whether $f \in \cb_\sigma$.  An algorithm is said to have \emph{stable computational cost} if 
...
Returning an approximation with the desired error is not enough.  One also wants the algorithm to be confident that the answer is correct.  An  algorithm  $(A,W) \in \ca(\cn,\ch,S,\Lambda)$, is deemed successful provided that it meets the prescribed error tolerance and does not raise the warning flag for attempting to exceed the cost budget, $N_{\max}$.  Specifically, success is defined as
\begin{multline*}
...
\comp(\varepsilon,\ca(\cn,\ch,S,\Lambda),N_{\max},\sigma) \\
 = \min\left\{\maxcost(A,\cn,\varepsilon,N_{\max},\sigma) : \success(A,W,\cn,\varepsilon,N_{\max}) = \text{true}, \right .\\
...
The set of non-zero, non-adaptive algorithms defined in the previous subsection is a subset of the automatic algorithms $\ca(\cf,\ch,S,\Lambda)$.  Algorithms in $\ca_{\fix}(\cf,\ch,S,\Lambda)$ are not affected by the error tolerance $\varepsilon$ and do not recognize a cost budget $N_{\max}$.  Moreover, the warning flag for an algorithm in $\ca_{\fix}(\cf,\ch,S,\Lambda)$ is always returned as false.  Whereas the non-adaptive algorithms are inherently impractical by themselves, they are vital components of automatic, adaptive algorithms.

\subsection{Cones of Functions} \label{conesubsec} All algorithms can be fooled by some input functions, even if these functions are sufficiently smooth.  Algorithm \ref{nonadaptalgo} rules out fooling functions with large error by restricting the size of  $\Fnorm{f}$.  

It is often difficult to know how large $\Fnorm{f}$ is a priori and so practical automatic algorithms try to bound it.  The framework described here rules out fooling functions whose $\cf$-semi-norms cannot be reliably bounded above.  This is done by considering $\cg$, a superspace of $\cf$, with its own semi-norm $\Gnorm{\cdot}$.   The semi-norm $\Gnorm{\cdot}$ is chosen to be weaker than $\Fnorm{\cdot}$, i.e., 
...
Theorems \ref{TwoStageDetermThm}, \ref{MultiStageThm}, \ref{multistageintegthm}, and \ref{multistageappxthm} provide upper bounds on the computational complexity of the problem as a function of $\varepsilon/\Fnorm{f}$.
...
\item \emph{Stable Computational Cost.}  Theorems  \ref{TwoStageDetermThm}, \ref{MultiStageThm}, \ref{multistageintegthm}, and \ref{multistageappxthm} provide positive results on the stability of the computational cost as defined in \eqref{coststabledef}.
...
Computing an approximate solution to the problem $S: \cc_{\tau} \to \ch$, e.g., integration or function approximation, depends on non-adaptive algorithms. Suppose that there is a sequence of such of algorithms, $\{A_n\}_{n \in \ci}$, with $A_n  \in \ca_{\fix}(\cg,\ch,S,\Lambda)$, indexed by their cost as defined in \eqref{algseqdef}, and for which upper error bounds are known $\cf$:
\begin{subequations} \label{algseqerrbd}
\begin{equation}
\err(A_n,\cf,\ch,S) \le h(n), 
\end{equation}
for some positive, \emph{non-increasing} function $h$ with $\inf_{n \in \ci} h(n)=0$.  Define the (non-increasing) inverse of $h$ as 
\begin{equation}
h^{-1}(\varepsilon) = \min \{n \in \ci : h(n) \le \varepsilon\}, \qquad \varepsilon > 0.
\end{equation}
This means that for all $n \in \ci$ and $\varepsilon > 0$.
\begin{equation}
h(n)\le\varepsilon \implies n \ge h^{-1}(\varepsilon), \qquad  h(n) > \varepsilon \implies  n < h^{-1}(\varepsilon).
\end{equation}
In this article we consider sequences of algorithms for which $h$ satisfies
\begin{equation}
\sup_{\epsilon > 0} \frac{h^{-1}(\varepsilon)}{h^{-1}(2\varepsilon)} < \infty.
\end{equation}
\end{subequations}
This means that $h(n)$ may decay polynomially in $n^{-1}$ or faster as $n$ tends to infinity.  The definitions of the errors in \eqref{errdefworst} then imply an upper bound on the error of $A_n(f)$ in terms of the $\cf$-semi-norm of $f$:
\begin{equation} \label{Anerrbound}
\norm[\ch]{S(f) -  A_n(f)} \le \err(A_n,\cf,\ch,S)\Fnorm{f}) \le \tau h(n)\Gnorm{f} \qquad \forall f \in \cc_{\tau}.
\end{equation}
...
Moreover, let  $\{A_n\}_{n \in \ci}$, $A_n  \in \ca_{\fix}(\cf,\ch,S,\Lambda)$, be a sequence of algorithms as described in \eqref{algseqdef} and \eqref{algseqerrbd}.  Given  a positive error tolerance, $\varepsilon$, and  an input function $f$, do the following:

\begin{description} 

\item[Step 1.\ Estimate {$\Gnorm{f}$}.] First compute $G_{n_G}(f)$.  Define the inflation factor $\fC=\fC_{n_G}$ according to \eqref{norminflate}.
Then $\fC G_{n_G}(f)$ provides a reliable upper bound on $\Gnorm{f}$.  

\item [Step 2.\ Estimate {$S(f)$}.] Choose the sample size need to approximate $S(f)$, namely, $n_A=h^{-1}(\varepsilon/(\tau\fC G_{n_G}(f)))$.
...
Otherwise, recompute $n_A$ to be within budget, $n_A = \tN_{\max} := \max\{n \in \ci : n\le N_{\max} -  n_G\}$, and set the warning flag, $W$ to true.  

Finally, compute $A_{n_A}(f)$ as the approximation to $S(f)$. Return the result $(A_{n_A}(f),W)$, at a total cost of $n_G+n_A$. 
\end{description}
\end{algo}

The bounds in Lemma \ref{Gnormlem} involving $G_n$ and $\Gnorm{\cdot}$ plus the bounds in \eqref{twosidedFGbound} involving $\Gnorm{\cdot}$ and $\Fnorm{\cdot}$ imply bounds on the desired number of samples in the algorithm above.  Since $h^{-1}$ is a non-increasing function, it follows that for all $f \in \cc_{\tau}$
\begin{equation*}
h^{-1}\left(\frac{\varepsilon}{\Fnorm{f}}\right) \le  h^{-1}\left(\frac{\varepsilon}{\tau\Gnorm{f}}\right)  \le h^{-1}\left(\frac{\varepsilon}{\tau \fC G_{n_G}(f)}\right) \le 
\begin{cases} \displaystyle  h^{-1}\left(\frac{\varepsilon}{\tau \fC \tfc \Gnorm{f}}\right) \\[2ex]
\displaystyle  h^{-1}\left(\frac{\tau_{\min} \varepsilon}{\tau \fC \fc \Fnorm{f}}\right)\end{cases}.
\end{equation*}
This means that proposed computational cost to compute $A(f)$ is reasonable, i.e., $ h^{-1}(\varepsilon/(\tau \fC G_{n_G}(f))) \le \tN_{\max}$ if either of the following conditions holds 
\begin{gather*}
\Gnorm{f} \le \varepsilon\tsigma_{\max}, \qquad \tsigma_{\max} := \frac{1}{\tau \fC \tfc h(\tN_{\max})}, \\
\Fnorm{f} \le \varepsilon\sigma_{\max}, \qquad  \sigma_{\max} := \frac{\tau_{\min}}{\tau \fC \fc h(\tN_{\max}))}.
\end{gather*}
...
\end{equation}
Since $\tN_{\max}$ is assumed to be huge, the radii $\tsigma_{\max}$ and $\sigma_{\max}$ are also huge.
...
\begin{gather}
n_G+ h^{-1}\left(\frac{\varepsilon}{\tau \Gnorm{f}} \right) \le 
\cost(A,f;\varepsilon,N_{\max})
\le n_G+ h^{-1}\left(\frac{\varepsilon}{\tau \fC\tfc\Gnorm{f}}\right), \\
n_G+ h^{-1}\left(\frac{\varepsilon}{\Fnorm{f}} \right) \le \cost(A,f;\varepsilon,N_{\max})
\le n_G+h^{-1}\left(\frac{\tau_{\min}\varepsilon}{\tau \fC\fc\Fnorm{f}}\right).
\end{gather}
\end{subequations}
Since $h$ satisfies \eqref{algseqerrbd}, then this algorithm is computationally stable in the sense that the maximum cost is no greater than some constant times the minimum cost.
\end{theorem}
...
\[
\norm[\ch]{S(f) -  A_n(f)} \le \tau \fC G_{n_G}(f) h(n) \qquad \forall f \in \cc_{\tau}.
\]
This error upper bound may be made no greater than the error tolerance, $\varepsilon$, by choosing the algorithm cost, $n$, to satisfy the condition in Step 2 of Algorithm \ref{twostagedetalgo}, provided that this can be done within the maximum cost budget.  In this case, the algorithm is successful, as claimed in the theorem.  
...
\begin{enumerate}
\renewcommand{\labelenumi}{\roman{enumi}.}
\item There must be an algorithm, $G_n$, that approximates the weaker semi-norm,  $\Gnorm{\cdot}$, and its error bound, $h_{\pm}$, as defined in \eqref{Gerrbds} must be known explicitly, as described in Section \ref{Galgosec}.
\item There must be a  sequence of nonadaptive algorithms, $\{A_n\}_{n \in \ci}$, as described at the beginning of  Section \ref{twostagesec}, and the error function $h$, defined in \eqref{algseqerrbd} must be known explicitly.  
\end{enumerate}
Sections \ref{integsec} and \ref{approxsec} provide concrete examples where these conditions are checked.
\end{rem}

\begin{rem} The maximum and minimum costs of Algorithm \ref{twostagedetalgo}, as given by \eqref{auto2stagedetcost}, depend on the $\cf$- and $\cg$-semi-norms of the input function, $f$.  However, the values of these semi-norms are not inputs to the algorithm, but rather are bounded by the algorithm.  The number of samples used to obtain the approximation to $S(f)$ is adjusted adaptively based on these bounds.
\end{rem}

\begin{rem}
The set of input functions for which the Algorithm \ref{twostagedetalgo} is guaranteed to work, $\cn$, is limited somewhat by the error tolerance, but only because of the practical constraint of a cost budget of $N_{\max}$.  This dependence disappears if one lifts this constraint by taking $N_{\max} \to \infty$.  The primary constraint determining the success of the algorithm is that $f$ lies in the cone $\cc_{\tau}$.
\end{rem}
...
\begin{rem} \label{neccondrem} In some cases it is possible to find a lower bound on the $\cf$-norm of the input function.  This means that there exists an algorithm $F_n$ using the same function values as $G_n$, such that
\[
...
We stress that this is not a sufficient condition for $f$ to lie in $\cc_{\tau}$, so it is possible to get an incorrect answer from Algorithm \ref{twostagedetalgo} or \ref{multistagealgo} even if the \eqref{neccondFn} is satisfied but $f \notin \cc_{\tau}$.  However, in light of this argument one might modify these algorithms by increasing $\tau$ to, say, $2 \tau_{\min,n}$ whenever $\tau$ falls below $\tau_{\min,n}$. 
\end{rem}

\begin{rem} \label{htilderem} It may also be possible to obtain an error upper bound in terms of weaker semi-norm, i.e., $\tildeh$ such that $\err(A_n,\cg,\ch,S) \le \tildeh(n)$. By following the argument leading to \eqref{Anerrbound}, it is clear that such an error bound is no use if
\begin{equation} \label{hbest}
\tau h(n) \le \tildeh(n) \quad \forall n \in \ci.
\end{equation}
This condition is observed to hold for the examples of Sections \ref{integsec} and \ref{approxsec}.  A sufficient condition for \eqref{hbest} is
\begin{equation} \label{hbestverb}
h(n) \le \tildeh(n) h_{+}(n) \quad \forall n \in \ci,
\end{equation}
since the Algorithms \ref{twostagedetalgo} and \ref{multistagealgo} both require that $h_{+}(n) < 1/\tau$, and since $h$ is a non-increasing function.   
\begin{comment} Another 
...
\item [Step 1. Estimate $\| f \| _{\cg}$.] Compute $G_{n_i}(f)$ and $\fC_{n_i} \le \infty$ as defined in \eqref{norminflate}.  
...
\end{equation}
and note that $h_1$ and $h_2$ are non-increasing functions.  Let $h_1^{-1}$ and $h_2^{-1}$ be defined analogously to $h^{-1}$ as in \eqref{algseqerrbd}.  These definitions imply that the quantity appearing on the left hand side of the convergence check, \eqref{multistageconv}, has the following upper and lower bounds based on \eqref{twosidedGineq} for $f \in \cc_{\tau}$:
\begin{subequations}\label{hh1h2bounds}
\begin{equation}
h(n)\Fnorm{f} \le \tau h(n)\Gnorm{f} \le \tau \fC_{n} h(n)G_{n}(f) \le \begin{cases} \tau h_1(n) \Gnorm{f} \\[1ex]
\displaystyle \frac{\tau h_2(n) \Fnorm{f} }{\tau_{\min}}
\end{cases} , \quad n \in \ci,
\end{equation}
\begin{multline}
h^{-1}\left(\frac{\varepsilon}{\Fnorm{f}}\right) \le h^{-1}\left(\frac{\varepsilon}{\tau \Gnorm{f}}\right) \le \min\{n : \tau \fC_{n} h(n)G_{n}(f) \le \varepsilon\} \\
\le \begin{cases} \displaystyle h_1^{-1}\left(\frac{\varepsilon}{\tau \Gnorm{f}} \right) \\[2ex]
...
\begin{theorem}  \label{MultiStageThm}  Let  $\cf$, $\cg$, $\ch$, $N_{\max}$, $\tau$, $n_1$, and $\varepsilon$ be given as described in Algorithm \ref{multistagealgo}. Let $r$ be the minimum cost multiple described in the paragraphs preceding that Algorithm \ref{MultiStageThm}. Assume that $n_1 \le N_{\max}/r$. 
...
\[
\cn = \cc_\tau \cap (\tcb_{\varepsilon\sigma_{1,\max}} \cup \cb_{\varepsilon \sigma_{2,\max}}).
\]
be the nice subset of the cone $\cc_\tau$, where the parameters determining the radii of the two balls are
\[
\sigma_{1,\max} = \frac{1}{\tau h_1(N_{\max}/r)}, \qquad \sigma_{2,\max} = \frac{\tau_{\min}}{\tau h_2(N_{\max}/r)}.
\]
Then it follows that Algorithm \ref{multistagealgo} is successful for all functions in this set of \emph{nice} functions $\cn$,  i.e.,  $\success(A,W,\cn,\varepsilon,N_{\max}) = \text{true}$.  Moreover, the cost of this algorithm is bounded above and below in terms of the unknown $\cg$- and $\cf$-semi-norms of any input function in $\cn$ as follows:
\begin{subequations}  \label{automultistagedetcost}
\begin{multline}
\max \left(n_1, h^{-1}\left(\frac{\varepsilon}{\tau \Gnorm{f}} \right) \right) \le 
\cost(A,f;\varepsilon,N_{\max}) \\
\le \max \left(n_1, rh_1^{-1}\left(\frac{\varepsilon}{\tau \Gnorm{f}} \right) \right),
\end{multline}
\begin{multline}
\max \left(n_1, h^{-1}\left(\frac{\varepsilon}{\Fnorm{f}} \right) \right) \le \cost(A,f;\varepsilon,N_{\max})\\
\le\max \left(n_1, rh_2^{-1}\left(\frac{\tau_{\min}\varepsilon}{\tau \Fnorm{f}} \right) \right).
\end{multline}
\end{subequations}
If $h$ satisfies \eqref{algseqerrbd}, then this algorithm is computationally stable in the sense that the maximum cost is no greater than some constant times the minimum cost.
\end{theorem}

\begin{proof} Let $n_1, n_2, \ldots, n_j$ be the sequence of $n_i$ generated by Algorithm \ref{multistagealgo}.  Here $j$ may be any positive integer, and must be finite because the design of the algorithm guarantees that $n_1 < \cdots < n_j \le N_{\max}$.  We shall prove that under the hypotheses of the theorem, in particular $f \in \cn$, that
\begin{enumerate}
\renewcommand{\labelenumi}{\roman{enumi})}
\item if the convergence criterion \eqref{multistageconv} is satisfied for $i$, then the algorithm stops and $A_{n_i}(f)$ is returned as the answer and it meets the errro tolerance, and
...
n_{i} \le 
\begin{cases} 
\displaystyle h_1^{-1}\left(\frac{\varepsilon}{\tau\Gnorm{f}}\right) \\[2ex]
\displaystyle h_2^{-1}\left(\frac{\tau_{\min}\varepsilon}{\tau\Fnorm{f}}\right)
\end{cases} .
\] 
The algorithm then considers the candidate $n^*_{i+1} = h^{-1}(\varepsilon \tfc_{n_{i}}/[\tau G_{n_{i}}(f)])$ as a possible choice for $n_{i+1}$.  If $n_{i} \ge n^*_{i+1}$, then Step 3 chooses $n_{i}$ to be the smallest element of $\ci$ that exceeds $n_{i-1}$ and for which $A_{n_{i}}$ is embedded in $A_{n_{i+1}}$.  By the definition of $r$ it follows that $n_{i+1} \le r  n_{i}$, and so by the above inequalities for $n_{i}$, it follows that $n_{i+1}$ is bounded above by the right hand sides of the inequalities in \eqref{automultistagedetcost}.
...
\begin{equation*}
n_{i+1} < r n^*_{i+1} = r h^{-1}\left(\frac{\varepsilon \tfc_{n_{j-1}}}{\tau G_{n_{j-1}}(f)}\right) \le r  h^{-1}\left(\frac{\varepsilon}{\tau \Gnorm{f}}\right) \le
\begin{cases}
\displaystyle  rh_1^{-1}\left(\frac{\varepsilon}{\tau \Gnorm{f}} \right) \\[2ex]
...
\end{align*}
Furthermore, applying \eqref{assumpfone}, it follows that any nonadaptive algorithm satisfying the error tolerance $\varepsilon$ must have a cost $n$ satisfying the following inequality:
\begin{align*}
...
\begin{equation*}
\comp(\varepsilon,\ca_{\fix}(\cf,\ch,S,\Lambda)) \ge
g ^{-1}(\varepsilon).
\end{equation*}
...
\begin{prop} \label{optimalprop} Suppose that there exist non-adaptive algorithms $\{A_n\}_{n \in \ci}$, with $A_n  \in \ca_{\fix}(\cg,\ch,S,\Lambda)$, for which the upper error bounds satisfy \eqref{algseqerrbd} for known $h$.  If 
\begin{equation}\label{ghcond}
\sup_{n \in \ci} \frac{h(n)}{g(n)} < \infty,
\end{equation}
then these algorithms are optimal in the sense of \eqref{nearoptdef} for the problem defined on $\cf$.
\end{prop}
...
\end{align*}
Thus, it follows from \eqref{algseqerrbd} that the error of $\{A_n\}_{n \in \ci}$ is no worse than a constant times the best possible non-adaptive algorithm:
\begin{align*}
\sup_{0 < \varepsilon \le 1} \frac{\min\{n \in \ci : \err(A_n,\cf,\ch,S) \le \varepsilon\}}{\comp(\varepsilon,\ca_{\fix}(\cf,\ch,S,\Lambda))}
& \le \sup_{0 < \varepsilon \le 1} \frac{\min\{n \in \ci : h(n) \le \varepsilon\}} {g^{-1}(\varepsilon)} \\
& \le \sup_{0 < \varepsilon \le 1} \frac{h^{-1}(\varepsilon)} {h^{-1}(C\varepsilon)} <\infty.
\end{align*}
\end{proof}

Now we turn to the solving the numerical problem where the functions lie in the intersection of the cone $\cc_{\tau}$ and the ball $\cb_{\sigma}$.  The cone condition makes the complexity lower bound a bit more difficult to derive.  Note that condition \eqref{assumpfone} does not require the fooling function $f_1$ to lie \emph{inside} this cone.  To remedy this shortcoming, the fooling function is constructed as a linear combination of $f_1$ and another function, $f_0$, lying in the interior of the cone.
Specifically, $f_0$ is assumed to satisfy
\begin{equation}
\label{assumpfzero}
\Gnorm{f_{0}} = 1, \qquad \Fnorm{f_{0}} \le \tau_{\min} \Gnorm{f_{0}} = \tau_{\min} < \tau.
\end{equation}
...
\begin{theorem} \label{complowbd} Assume that $\tau > \tau_{\min}$ and let $\sigma$ be some positive number.  Suppose that functions $f_{0}$ and $f_1$ can be found that satisfy conditions \eqref{assumpfone} and \eqref{assumpfzero}.  It then follows that the complexity of the problem, defined by \eqref{complexdef}, assuming infinite cost budget, over the intersection of the cone $\cc_{\tau}$ with the ball $\cb_{\sigma}$ is
\begin{equation*}
\comp(\varepsilon,\ca(\cc_{\tau}\cap \cb_{\sigma},\ch,S,\Lambda),\infty,\cb_{\sigma}) 
\ge g^{-1}\left(\frac{2 \tau \varepsilon}{\sigma(\tau-\tau_{\min})}\right).
\end{equation*}
\end{theorem}

\begin{proof} Let $A$ be a successful, possibly adaptive, algorithm for all functions lying in $\cc_{\tau}\cap \cb_{\sigma}$.  Given an error tolerance, $\varepsilon$, and a positive $\sigma$, let $f_0$ be a function satisfying \eqref{assumpfzero} and choose 
\begin{subequations}\label{c0c1bumpdef}
\begin{equation} 
\label{c0bumpdef}
c_0 = \frac{\sigma(\tau+\tau_{\min})}{2\tau\tau_{\min}} > 0.
\end{equation} 
...
\begin{equation} 
\label{c1bumpdef}
c_1 = \frac{\sigma (\tau-\tau_{\min}) g(n)}{2\tau } > 0.
\end{equation}
\end{subequations}
...
& \le \frac{c_1}{g(n)} \left(1+\frac{\tau}{\tau_{\min}}\right) - c_{0}  (\tau - \tau_{\min}) \qquad \text{by \eqref{assumpfone}, \eqref{assumpfzero}} \\
& = 0 \qquad \qquad \text{by \eqref{c0c1bumpdef}}.
\end{align*}
Moreover, both fooling functions have $\cf$-semi-norms no greater than $\sigma$, since
\begin{align*}
\Fnorm{f_{\pm}} &\le c_{0} \Fnorm{f_0} + c_1 \Fnorm{f_1} \\
& = \frac{\sigma}{2\tau}\left[\frac{\tau+\tau_{\min}}{\tau_{\min}}\tau_{\min} + (\tau-\tau_{\min})g(n)  \frac{1}{g(n)} \right] \qquad \text{by \eqref{c0c1bumpdef}}\\
&\le \frac{\sigma}{2\tau} 2 \tau = \sigma.
\end{align*}
...
\[
\frac{2\tau\varepsilon}{\sigma(\tau-\tau_{\min})}  \ge \frac{2\tau c_1} {\sigma(\tau-\tau_{\min})}  = g(n) .
\]
...
The theorem above provides a lower bound on the complexity of the problem when the integrand is known to lie both in the cone and in a $\cf$-ball of given radius.  This automatically provides a lower bound on the complexity if no upper bound on the $\cf$-semi-norm of the integran is known.
\begin{cor} \label{compconecor}
Under the hypotheses of Theorem \ref{complowbd}
\begin{equation*}
\comp(\varepsilon,\ca(\cc_{\tau},\ch,S,\Lambda),\infty,\cb_{\sigma}) 
\ge g^{-1}\left(\frac{2 \tau \varepsilon}{\sigma(\tau-\tau_{\min})}\right).
\end{equation*} 
\end{cor}
...
\toprule
{\bf Minimum} cost of the {\bf best} algorithm that knows that $f \in \cc_{\tau}$ and $\Fnorm{f}$ & $\displaystyle \ge g^{-1}\left(\frac{2 \tau \varepsilon}{\Fnorm{f}(\tau-\tau_{\min})}\right)$ \tabularnewline
\midrule
{\bf Minimum} cost of the {\bf best} algorithm that knows that $f \in \cc_{\tau}$, but not $\Fnorm{f}$ & $\displaystyle \ge g^{-1}\left(\frac{2 \tau \varepsilon}{\Fnorm{f}(\tau-\tau_{\min})}\right)$ \tabularnewline
\midrule
Cost of a {\bf nonadaptive} algorithm (like Algorithm \ref{nonadaptalgo}) that knows $\Fnorm{f}$ & $\displaystyle h^{-1}\left(\frac{\varepsilon}{\Fnorm{f}}\right)$ \tabularnewline
\midrule
{\bf Minimum} cost of {\bf adaptive} Algorithm \ref{multistagealgo} that knows $f \in \cc_{\tau}$, \\ but not $\Fnorm{f}$ & $\displaystyle \max\left( n_1, h^{-1}\left(\frac{\varepsilon}{\Fnorm{f}}\right)\right)$ \tabularnewline
\midrule
{\bf Maximum} cost of {\bf adaptive} Algorithm \ref{multistagealgo} that knows $f \in \cc_{\tau}$, \\ but not $\Fnorm{f}$ & $\displaystyle \max \left(n_1, rh_2^{-1}\left(\frac{\tau_{\min}\varepsilon}{\tau \Fnorm{f}} \right) \right)$ \tabularnewline
\bottomrule
\end{tabular}
\caption{Costs of various algorithms, $A$, guaranteed to satisfy the tolerance, i.e.,  $\norm[\ch]{S(f) - A(f)} \le \varepsilon$. \label{costcomparefig}}
\end{table}

Table \ref{costcomparefig} summarizes the lower and upper bounds on the computational complexity of computing $S(f)$ to within an absolute error of $\varepsilon$.  The results summarized here are based on Theorems \ref{NonAdaptDetermThm}, \ref{MultiStageThm}, and \ref{complowbd}, and Corollary \ref{compconecor}.

Comparing the lower bound on the computational complexity in Corollary \ref{compconecor} to the upper bounds on the cost of the adaptive algorithms in Theorems \ref{TwoStageDetermThm} and \ref{MultiStageThm}, one can see that these adaptive algorithms are optimal for solving the problem for a cone of input functions if their non-adaptive building blocks are also optimal for solving the problem for balls of input functions.  In fact, under the hypotheses of Proposition \ref{optimalprop} all the algorithms mentioned in Table \ref{costcomparefig}, from the best ideal ones to the actual ones, have essentially the same computational cost.

\begin{cor} \label{optimcor}
Suppose that the functions $g$ and $h$ satisfy the hypotheses of Proposition \ref{optimalprop}, i.e., conditions \eqref{ghcond}, which means that the non-adaptive algorithms are optimal for solving the problem on $\cf$-balls of input functions.  It then follows that Algorithms \ref{twostagedetalgo} and \ref{multistagealgo} are both optimal for solving the  problem on the cone $\cc_{\tau}$.  
\end{cor}
...
\item the upper bound error function, h, defined in \eqref{algseqerrbd}, 
