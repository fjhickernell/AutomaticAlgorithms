...
\begin{algo} \label{nonadaptalgo} {\bf (Automatic, Non-Adaptive).} Let $\cf$ and $\ch$ be Banach spaces as described above, let $S$ be the solution operator, and let $\sigma$ be a fixed positive number.  Let $\{A_n\}_{n \in \naturals}$ be a sequence of algorithms as described above such that error bound \eqref{traditionerra} holds.  Let  $\varepsilon$ a positive error tolerance and $f \in \cf$ be an input function.  Compute the computational cost needed to satisfy the error tolerance:
\begin{equation}
\label{costupbd}
...
\item If, as often is the case, $G_{n_G}(f)$ does not overestimate the $\cg$-semi-norm of $f$.  Moreover, since $\Gnorm{\cdot}$ is a weaker semi-norm, it follows that $\tau_{\min} \Gnorm{f} \le \Fnorm{f}$.  Thus, one can be assured that the above criterion will be met for 
\[
n=\cost(A_n)\le \left \lceil\left(\frac{ C_{\up}\tau \Fnorm{f})}{\varepsilon \tau_{\min} (1 - \tau h_+(n_G))}\right)^{1/p} \right \rceil.
\]
Thus, the cost of this adaptive aglgorithm is at worst a constant multiple times the cost of the non-adaptive Algorithm \ref{nonadaptalgo} that requires an a priori upper bound on $\Fnorm{f}$.
...
Section \ref{integsec} presents a concrete example of this approach for the problem of computing the integral $\int_0^1 f(x) \, \dif x$ for all integrands whose derivatives have finite total variation, i.e., $\Fnorm{f}=\Var(f')$. The trapezoidal rule based on $n$ function values, i.e., $n-1$ trapezoids, computes an approximation with error no greater than $\varepsilon$ using $2+ \sqrt{\Var(f')/(8\varepsilon)}$ function values.  However, except for relatively simple integrands, a bound on $\Var(f')$ is unknown in practice.  

Following the procedure outlined above we choose the weaker semi-norm as $\Gnorm{f} = \norm[1]{f'-f(1)+f(0)}$.  This semi-norm may be estimated by an algorithm $G_n$ that takes the $\cg$-semi-norm of the piecewise linear spline for $f$.  The error for $G_n(f)$ is bounded by $\Var(f')/(2n-2)$.  This implies that $2\norm[1]{f'-f(1)+f(0)} \le \Var(f')$ for all $f$ whose derivative has finite total variation.  Fixing the parameter $\tau > 2$, and assuming that $\norm[1]{f''} \le \tau \norm[1]{f'-f(1)+f(0)}$  (the cone condition), then leads to a guaranteed, automatic (adaptive) algorithm for approximating the integral to the desired accuracy with a computational cost that is no greater than $4+ \tau + \sqrt{\tau \Var(f')/(4\varepsilon)}$ (Theorem \ref{multistageintegthm}), where $\Var(f')$ is unknown a priori.  Here $\tau$ represents a minimum sample size, and $1/\tau$ represents a length scale of for possible spikes that one wishes to integrate accurately.
...
\begin{equation}\label{algseqerrbddef}
\err(A_n,\cf,\ch,S) \le h(n), 
\end{equation}
for some \emph{non-increasing} function $h$.  Define the (non-increasing) inverse of $h$ as 
\begin{equation}
h^{-1}(\varepsilon) = \min \{n \in \ci : h(n) \le \varepsilon\}.
\end{equation}
...
\end{equation}
\end{subequations}
This means that $h(n)$ may decay polynomially in $n^{-1}$ or faster as $n$ tends to infinity.  The definitions of the errors in \eqref{errdefworst} then implies an upper bounds on the error of $A_n(f)$ in term of the $\cf$-semi-norm of $f$:
...
\item [Stage 2.\ Estimate {$S(f)$}.] Choose the sample size need to approximate $S(f)$, namely, $n_A=h^{-1}(\varepsilon/(\tau\fC G_{n_G}(f)))$.
...
\item There must be a  sequence of nonadaptive algorithms, $\{A_n\}_{n \in \ci}$, as described at the beginning of  Section \ref{twostagesec} and satisfying condition \eqref{algseqerrbd}, and the error function $h$, defined in \eqref{algseqerrbd} must be known explicitly.  
\end{enumerate}
Sections \ref{integsec} and \ref{approxsec} provide concrete examples where these conditions are checked.
\end{rem}

\begin{rem} The maximum and minimum costs of Algorithm \ref{twostagedetalgo}, as given by \eqref{auto2stagedetcost}, depend on the $\cf$- and $\cg$-semi-norms of the input function, $f$.  However, the values of these semi-norms are not required as inputs to the algorithm, but rather are conservatively estimated by the algorithm.  The number of samples used to obtain the approximation to $S(f)$ is adjusted adaptively based on these bounds on the semi-norms.
\end{rem}
...
\begin{rem} It may also be possible to obtain an error upper bound, $\tildeh$ such that $\err(A_n,\cg,\ch,S) \le \tildeh(n)$. By following the argument leading to \eqref{Anerrbound}, it is clear that such an error bound is no use if
...
h_1(n) = \fC_n\tfc_n h(n), \quad h_2(n) = \fC_n\fc_n h(n) \qquad n \in \ci.
\end{equation}
and note that the left hand side of the convergence check, \eqref{multistageconv}, has the following upper and lower bounds based on \eqref{twosidedGineq} for $f \in \cc_{\tau}$:
\begin{equation} \label{hh1h2bounds}
h(n_i)\Fnorm{f} \le \tau h(n_i)\Gnorm{f} \le \tau \fC_{n_i} h(n_i)G_{n_i}(f) \le \begin{cases} \tau h_1(n_i) \Gnorm{f} \\[1ex]
\displaystyle \frac{\tau h_2(n_i) \Fnorm{f} }{\tau_{\min}}
\end{cases} .
\end{equation}
...
\[
\cn = \cc_\tau \cup (\tcb_{\varepsilon\sigma_{1,\max}} \cup \cb_{\varepsilon \sigma_{2,\max}}).
\]
be the nice subset of the cone $\cc_\tau$, where the parameters determining the radii of the two balls are
\[
\sigma_{1,\max} = \frac{\varepsilon}{\tau h_1(N_{\max}/r)}, \qquad \sigma_{2,\max} = \frac{\varepsilon}{\tau h_2(N_{\max}/r)}.
\]
...
\le\max \left(n_1, rh_1^{-1}\left(\frac{\tau_{\min}\varepsilon}{\tau \Fnorm{f}} \right) \right).
\end{multline}
\end{subequations}
If $h$ satisfies \eqref{algseqerrbd}, then this algorithm is computationally stable in the sense that the maximum cost is no greater than some constant times the minimum cost.
\end{theorem}
...
If $j=1$, then the cost of the algorithm is $n_1$.  If $j>1$, then the convergence criterion was not satisfied for $i=j-1$. Thus, it follows that $n_{j-1} < h_1^{-1}(\varepsilon/(\tau\Gnorm{f}))$ by the inequality in \eqref{hh1h2bounds}. 
