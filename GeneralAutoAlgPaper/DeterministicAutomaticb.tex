...
\item[Automatic]--- the computational cost for a given input is determined \emph{adaptively}, based on $\varepsilon$ and on the difficulty of the problem (size of the input function) \emph{as inferred from data, not specified a priori}.
...
\end{description}
This article presents a general framework for constructing  algorithms matching the above criteria and also provides two concrete examples, namely, univariate integration via the composite trapezoidal rule and univariate function recovery via linear splines.
...
\end{gather}
\end{subequations}
Furthermore, the computational cost of the algorithm, $\cost(A)$, which typically corresponds to the number of function data needed, is bounded in terms of $\varepsilon$ and the radius of the ball, $\sigma$.  Figure \ref{fig:NonadaptAlgo} gives a diagram of this algorithm.  It is said to be optimal if any algorithm satisfying the error criterion above must have a cost of at least $\Order((\sigma/\varepsilon)^{1/p})$.
...
A practical drawback of the algorithm described above is that the algorithm requires as input, $\sigma$, a known upper bound on $\norm[\cf]{f}$, which might be called the \emph{difficulty of the problem}. Moreover, the cost of such an algorithm is fixed and depends on $\sigma$.  The computational cost does not decrease if $\norm[\cf]{f}$ is drastically smaller than $\sigma$. 
...
\cc_{\tau}=\{f \in \cf : \norm[\cf]{f} \le \tau \norm[\cg]{f} \}.
\end{equation}
In essence, functions inside the cone may have large norms, but their $\cg$-semi-norm can be estimated with reasonable effort.  The argument to obtain an approximation to the solution, $S(f)$, with a guaranteed error estimate from data proceeds as follows (in somewhat simplified form):
\begin{itemize}
...
\begin{lem} \label{Gnormlem} Any nonadaptive algorithm $G_n \in \ca_{\fix}(\cf,\reals_+,\norm[\cg]{\cdot},\Lambda)$ with cost $n=\cost(G_n)$ and two sided error bounds $\err_{\pm}(G_n,\cf,\reals_{+},\norm[\cg]{\cdot}) \le h_{\cg\pm}(n)$ yields an approximation to the $\cg$-semi-norm of functions in the cone $\cc_{\tau}$ with the following upper and lower error bounds:
