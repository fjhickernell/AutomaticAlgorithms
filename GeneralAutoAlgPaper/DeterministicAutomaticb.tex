...
\item[Automatic]--- the computational cost for a given input is determined \emph{adaptively}, based on $\varepsilon$ and on the difficulty of the problem (size of the input function) \emph{as inferred from data}.
...
\end{description}
This article presents a general framework for constructing such algorithms and also provides two concrete examples, namely, univariate integration and univariate function recovery.
...
\cb_\sigma=\{ f \in \cf : \norm[\cf]{f} \le \sigma\}.
\end{equation}
Here it is necessarily assumed that the algorithm is exact if the semi-norm of the input function vanishes, i.e., $S(f)=A(f)$ if $\norm[\cf]{f}=0$.  Furthermore, the computational cost of the algorithm, $\cost(A)$, which typically corresponds to the number of function data needed, is bounded in terms of $\varepsilon$ and the radius of the ball, e.g.,
\begin{equation} \label{costupbd}
...
A practical drawback of the analysis summarized above is that the definition of the algorithm, at least as far as its cost is concerned, depends on knowing a priori an upper bound on $\norm[\cf]{f}$.  This norm of the input function may be thought of as the \emph{difficulty of the problem}.  The algorithm needs the radius of the ball, $\sigma$, as an input. Moreover, the cost of such an algorithm is fixed and depends on $\sigma$. 

Automatic algorithms try to estimate $\norm[\cf]{f}$, so that the computational cost can be adjusted accordingly. This is the approach taken here.  Some automatic algorithms approximate $\norm[\cf]{f}$ heuristically or provide an estimate which converges asymptotically as the number of function values tends to infinity.  The aim here is to find from data a \emph{rigorous} upper bound on $\norm[\cf]{f}$.  

This article shows how to obtain such an upper bound for functions lying in a \emph{cone} instead of a ball.  Let $\cg$ be some superspace of $\cf$ with a weaker semi-norm, and define the cone
\begin{equation} \label{conedef}
\cc_{\tau}=\{f \in \cf : \norm[\cf]{f} \le \tau \norm[\cg]{f} \}.
\end{equation}
The argument to obtain an approximate solution to the problem with a reliable error estimate from data proceeds as follows:
\begin{itemize}
\item Since the $\cg$-semi-norm is weaker than the $\cf$-semi-norm, it can be approximated by some algorithm $G(f)$, with $\norm[\cg]{f}-G(f) \le C_1\norm[\cf]{f}$ for some known $C_1$.

\item The definition of the cone in \eqref{conedef} implies that $\norm[\cg]{f} \le G(f)/(1 - \tau C_1)$, provided that $\tau< C_1$.

\item Applying the cone condition again,  $\norm[\cf]{f} \le \tau G(f)/(1 - \tau C_1)$.

\item This data-driven upper bound on $\norm[\cf]{f}$ can be used with \eqref{costupbd} to guarantee that the error tolerance will be met with 
\[
\cost(A)\le C_{\up}\left(\frac{\tau G(f)}{\varepsilon(1 - \tau C_1)}\right)^{p}.
\]

\end{itemize} 

Section \ref{integsec} presents the concrete example of this approach for the problem of computing the integral $\int_0^1 f(x) \, \dif x$ for all integrands whose second derivatives are absolutely integrable, i.e., $\norm[\cf]{f}=\norm[1]{f''}$.  The trapezoidal rule computes an approximation with error no greater than $\varepsilon$ using $\Order(\sqrt{\norm[1]{f''}/\varepsilon})$ function values.  However, except for relatively simple integrands, $\norm[1]{f''}$ is unknown in practice.  One may estimate $\norm[1]{f'}$ by the one-norm of the derivative of the piecewise linear spline for $f$, and the error of approximating $\norm[1]{f'}$ in this way can be bounded rigorously in terms of $\norm[1]{f''}$.  If one chooses a $\tau > 1$ and assumes that $\norm[1]{f''} \le \tau \norm[1]{f'}$  (the cone condition), then the reliable numerical upper bound on $\norm[1]{f'}$ can be used as a surrogate for $\norm[1]{f''}$.  This leads to a guaranteed, automatic (adaptive) algorithm for approximating the integral to the desired accuracy.
