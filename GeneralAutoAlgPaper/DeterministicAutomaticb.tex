...
\begin{proof} Let $n_1, \ldots, n_{j+1}$ be the sequence of $n_i$ generated by Algorithm \ref{multistagealgo}, with $n_{j+1}$ being the final value before termination. Here $j$ may be any non-negative integer.  The stages of Algorithm \ref{multistagealgo} guarantee that $n_1 < \cdots < n_j \le n_{j+1}$, with equality of the last two terms only in the case that $n_{j+1}=N_{\max}$ and $W=1$.  Moreover, this sequence does terminate since $n_{j+1}$ may not exceed $N_{\max}$. 

Note that by Lemma \ref{Gnormlem} and by \eqref{frakCn}, it follows that
\begin{equation}\label{twosidedGineq}
\frac{G_{n}(f)}{\fc_n} \le \norm[\cg]{f} \le \fC_{n}G_{n}(f) \qquad \forall n \in \ci.
\end{equation}
Note also that $n_1$ does not exceed the cost budget.

At iteration $i$, there are two possibilities.  If the convergence condition, \eqref{multistageconv} is satisfied for $n_i$, then $\norm[\ch]{S(f)-A_{n_i}(f)} \le \varepsilon$ by the same argument as given in the proof of Theorem \ref{TwoStageDetermThm}.  In this case the algorithm terminates successfully.  


Moreover, the inequality above implies that the convergence criterion must be satisfied for any $n_i \ge \tN_A(\varepsilon/\norm[\cg]{f})$, since $h(n), \tildeh(n), \fC_n$, and $\fc_n$ all do not decrease as $n$ increases.  

It now needs to be shown that for $f \in \cn$, $n_i$ does indeed increase to the point that \eqref{multistageconv} is satisfied.  Suppose that $n_i$ is not large enough to satisfy the convergence condition, \eqref{multistageconv}. It is now shown that $n_{i+1}$If $n_i < N_{A}(\varepsilon \fc_{n_i}/G_{n_i}(f))$, then Stage 3 chooses $n_{i+1}$ to satisfy 
\begin{align*}
n_i < N_{A}(\varepsilon \fc_{n_i}/G_{n_i}(f)) & \le n_{i+1} \qquad \text{procedure in Stage 3} \\
& \le r  N_{A}(\varepsilon \fc_{n_i}/G_{n_i}(f)) \qquad \text{definition of $r$}\\
& \le r  N_{A}(\varepsilon/\norm[\cg]{f}) \qquad \text{by \eqref{twosidedGineq}} \\
& \le r \tN_{A}(\varepsilon/\norm[\cg]{f}) \qquad \text{definition of $\tN_A$}
\end{align*}
Alternatively, if $N_{A}(\varepsilon \fc_{n_i}/G_{n_i}(f)) \le n_i$, then 
\begin{align*}
N_{A}(\varepsilon \fc_{n_i}/G_{n_i}(f)) \le n_i & < n_{i+1} \qquad \text{procedure in Stage 3} \\
& \le r  n_i \qquad \text{definition of $r$}\\
& < r \tN_{A}(\varepsilon/\norm[\cg]{f}) \qquad \text{since $n_i$ was not large enough}
\end{align*}
In both cases $n_{i+1} \le r \tN_{A}(\varepsilon/\norm[\cg]{f}) \le N_{\max}$ by the definition of $\cn$.  Thus, $n_{i+1}$ is the new sample size, not exceeding the cost budget.


From the definition Algorithm \ref{multistagealgo} $n_i$ never becomes and  some point 


At the $i^{\text{th}}$ iteration of  Algorithm \ref{multistagealgo}, $n_i \le N_{\max}$ by definition or assumption.  Thus, if the convergence condition, \eqref{multistageconv} is satisfied, then $\norm[\ch]{S(f)-A_{n_i}(f)} \le \varepsilon$ by the same argument as given in the proof of Theorem \ref{TwoStageDetermThm}.  In this case the algorithm terminates successfully.

If at the $i^{\text{th}}$ iteration of Algorithm \ref{multistagealgo}, the convergence condition is not satisfied, then $n_{i+1}$ is chosen according to Stage 3. It is claimed that
\[
n_{i+1} \le r N_{\min}(\varepsilon \fc_i/G_i(f)) \le r N_{\min}(\varepsilon/\norm[\cg]{f}). 
\]
The second inequality follows by the definition of $\fc_i$.  If the first inequality holds because for  it would imply that there exists an $n_i^{(1)}< N_{\min}(\varepsilon \fc_i/G_i(f))$ that has $A_{n}$ If the firstLet $n_i^(1), n_i^(2), \ldots$ be a sequence of numbers, such that $A_{n_i^{(j+1)}}$ to be no greater than $rN_{\min}(\varepsilon \fc_i/G_i(f))$, which 



 in Stage 2, we have The definition of $\fC$ in \eqref{norminflate} implies that the true $\cg$-semi-norm of $f$ is bounded above by $\fC G(f)$ according to Lemma \ref{Gnormlem}.  The upper bound on the error of the sequence of algorithms $\{A_n\}_{n \in \ci}$ in \eqref{Anerrbound} then implies that 
\[
\norm[\ch]{S(f) -  A_n(f)} \le \min(h(n),\tau \tildeh(n)) \fC G(f) \qquad \forall f \in \cc_{\tau}.
\]
This error upper bound may be made no greater than the error tolerance, $\varepsilon$, by choosing the algorithm cost, $n$, to satisfy the condition in Stage 2. of Algorithm \ref{twostagedetalgo}, provided that this can be done within the maximum cost budget.  In this case, the algorithm is successful, as claimed in the theorem.
