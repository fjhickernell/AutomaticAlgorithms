...
To derive the error bounds for $A_n(f)$, note that for $x_{i} \le x \le x_{i+1}$ the error can be bounded in terms of the norm of the first derivative as
\begin{align*}
\abs{f(x)-A_n (f)(x)} &= \abs{f(x) - (n-1) \left[f(x_{i})(x_{i+1}-x) +f(x_{i+1})(x-x_i) \right]}\\
&= (n-1)\left \lvert (x_{i+1}-x) \int_{x_i}^x f'(t) \, \dif t - (x-x_i)  \int_x^{x_{i+1}} f'(t) \, \dif t\right \rvert \\
&= 2 (n-1)(x-x_i)(x_{i+1}-x)\sup_{x_{i} \le x \le x_{i+1}}\abs{f'(x)} \\
&\le \frac{\norm[\infty]{f'}}{2(n-1)}
\end{align*}
The error can also be bounded in terms of the norm of the second derivative as
\begin{align*}
\abs{f(x)-A_n(f)(x)} &= \abs{f(x) - (n-1) \left[f(x_{i})(x_{i+1}-x) +f(x_{i+1})(x-x_i) \right]}\\
&= (n-1)\left \lvert \int_{x_i}^x f''(t)(t-x_i)(x_{i+1}-t) \, \dif t \right \rvert \\
&= 2 (n-1)(x-x_i)(x_{i+1}-x)\sup_{x_{i} \le x \le x_{i+1}}\abs{f'} \\
&\le \frac{\norm[\infty]{f'}}{2(n-1)}
\end{align*}
