...
\begin{align}
\text{Definition} \quad & T_k(x) = \cos(k \theta), \nonumber \\
& \qquad \qquad \theta=\cos^{-1}(x), \quad -1 \le x \le 1, \quad \text{\citep[18.5.1]{OlvEtal10a}} \\
\text{Degree} \quad &T_k(x) \text{ is a polynomial of degree } k \in \natzero  \nonumber \\
\text{Orthogonality} \quad & \int_{-1}^{1} \frac{T_k(x)T_{l}(x)}{\sqrt{1-x^2}} \, \dif x = \frac{\delta_{k,l}\pi}{2-\delta_{k,0}} , \nonumber \\ 
& \qquad \qquad k,l \in \natzero \quad \text{\citep[Table 18.3.1]{OlvEtal10a}} 
\end{align}
