...
\end{equation}
The error bound for the trapezoidal rule in \eqref{traperrbd} and the error bound on the error estimate in \eqref{Simperrbd} imply that unless $n$ is small, $\Var(f')$ and $\Var(f''')$ must both be large for the integrands satisfying \eqref{failcond}.
Figure \ref{spikeflukefig} shows two such integrands for $n=16$. These two integrands illustrate two different ways in which the error estimate $\herr(\cdot)$ fails, and hence, Algorithm \ref{baderralgo} also fails.  One way is ultimately unavoidable but should be quantifiable.  The other way is inexcusable, but is prevalent in widely used automatic numerical quadrature.
