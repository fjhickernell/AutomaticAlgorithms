...
\end{equation}
(see \ocite{BraPet11a}*{Sect.\ 7.2, (7.15)}). The trapezoidal rule gives the exact answer for linear integrands.  Error bound \eqref{traperrbd} reflects this fact since if $f(x)=\alpha x+ \beta$, then $f'(x)=\alpha$, and so $\Var(f')=0$.
...
\includegraphics[width=5.5cm]{ProgramsImages/SpikyInteg16TrapFigcolor.eps} 
&
\includegraphics[width=5.5cm]{ProgramsImages/FlukyInteg16TrapFigcolor.eps} \tabularnewline
(a) & (b)
\end{tabular}
...
Unfortunately, these error estimates are unreliable, as pointed out by James Lyness \ycite{Lyn83}.  To illustrate the flawed approach to error estimation in even sophisticated adaptive quadrature algorithms, we focus an adaptive trapezoidal algorithm.
